...
public class TriangleTests {
	...
	public void testNormal() {
		Triangle tr = new Triangle(new Point3D(1, 1, 1), new Point3D(1, 2, 1), new Point3D(2, 4, 1), Color.WHITE,
				new Material(1, 1, 0, 0, 1));
		assertEquals(new Vector(0, 0, -1), tr.getNormal(Point3D.ZERO), "Bad GetNormal");
	}

	@Test
	public void testFindIntersections() {
		Triangle tr = new Triangle(new Point3D(1, 1, 0), new Point3D(4, 1, 0), new Point3D(4, 4, 0), Color.WHITE,
				new Material(1, 1, 0, 0, 1));
		...
		assertEquals(null, tr.findIntersections(interay),
				"Bad FindIntersections outside the triangle in the continue of the edge");
		// 2 - outside the triangle in the trapped part
		interay = new Ray(new Point3D(2, 0.5, 1), new Vector(0, 0, -1));
		assertEquals(null, tr.findIntersections(interay),
				"Bad FindIntersections outside the triangle in the trapped part");
		// 3 - at the edge
		...
		assertEquals(null, tr.findIntersections(interay),
				"Bad FindIntersections outside the triangle at the trapper part");
	}
}
