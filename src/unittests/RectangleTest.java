...
class RectangleTest {
	...
	public void testNormal() {
		Rectangle rec = new Rectangle(new Point3D(0,1,0),new Point3D(0,0,0),new Point3D(1,0,0),Color.WHITE,new Material(1,1,1));
		assertEquals(
				  new Vector(0, 0, -1),
				  rec.getNormal(Point3D.ZERO),
				  "Bad GetNormal");
	}
	...
	public void testFindIntersections() {
		...
		assertEquals(null, rec.findIntersections(interay),"Bad FindIntersections outside the rectangle in the continue of the edge");
		//2 - outside the rectangle in the trapped part
		interay =new Ray(new Point3D(-0.5,-0.5,1),new Vector(0,0,-1));
		assertEquals(null, rec.findIntersections(interay),"Bad FindIntersections outside the rectangle in the trapped part");
		//3 - at the edge
		interay =new Ray(new Point3D(0.5,0,1),new Vector(0,0,-1));
		assertEquals(null, rec.findIntersections(interay),"Bad FindIntersections at the edge");
		...
		interay =new Ray(new Point3D(0.5,-0.5,1),new Vector(0,0,-1));
		assertEquals(null, rec.findIntersections(interay),"Bad FindIntersections outside the rectangle at the trapper part");
	}
